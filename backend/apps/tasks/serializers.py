...
from .models import Task, Comment, TaskLifecycleStage, TaskPriority
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    collaborator_ids = serializers.PrimaryKeyRelatedField(queryset=User.objects.all(), source='collaborators', write_only=True, many=True, required=False)
    ...
    class Meta:
        model = Task
        fields = ['id', 'title', 'description', 'status', 'priority','assignee','assignee_id','collaborators','collaborator_ids','creator','deadline', 'created_at', 'updated_at']
        read_only_fields = ['creator', 'created_at', 'updated_at']
    def create(self, validated_data):
        request = self.context.get('request', None)
        if request: validated_data['creator'] = request.user
