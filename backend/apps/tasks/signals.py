...
def log_task_changes_and_notify(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        current_s_d = instance.get_status_display()
        prev_s_d = dict(TaskLifecycleStage.choices).get(previous.get('status'), '?')
        current_p_d = instance.get_priority_display()
        prev_p_d = dict(TaskPriority.choices).get(previous.get('priority'), '?')

        if instance.title != previous.get('title'):
            changes.append(f"Title changed to '{instance.title}'")
        if instance.description != previous.get('description'):
            changes.append("Description updated")
        if instance.status != previous.get('status'):
            changes.append(f"Status changed from '{prev_s_d}' to '{current_s_d}'")
        if instance.priority != previous.get('priority'):
            changes.append(f"Priority changed from '{prev_p_d}' to '{current_p_d}'")
        if instance.assignee != previous.get('assignee'):
             ...
             if instance.assignee and instance.assignee != previous.get('assignee'):
                 Notification.objects.create(
                     recipient=instance.assignee,
                     message=f"You have been assigned task: '{instance.title}'",
                     related_task=instance
                 )
        if instance.deadline != previous.get('deadline'):
            deadline_str = instance.deadline.strftime('%Y-%m-%d') if instance.deadline else "None"
            changes.append(f"Deadline changed to {deadline_str}")
