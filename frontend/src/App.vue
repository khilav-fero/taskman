<template>
  <v-app id="taskman-app">
    ...
    >
      <v-list-item
        lines="two"
        :prepend-avatar="currentUser?.profile?.avatar_url || 'https://via.placeholder.com/40/' + ($vuetify.theme.current.colors.primary.substring(1)) + '/FFFFFF?text=' + (currentUser?.username?.[0]?.toUpperCase() || 'U')"
        nav
        class="user-profile-list-item pa-2 mx-1 mt-1"
        min-height="68"
        :title="(!rail || !lgAndUp) ? (currentUser?.username || 'User') : undefined"
...
const shouldShowSidebar = computed(() => {
  return isAuthenticated.value && !route.meta.hideSidebar;
});
...
async function handleLogout() {
  await performLogoutActions();
  if (!lgAndUp.value && drawerVisibleState.value) {
    drawerVisibleState.value = false;
  }
